...
6. 모든 움직임은 valid하다는 가정이 있다. 즉,
  1. 개구리가 움직일 때에는 반드시 조건에 맞는(오른쪽에 있는 비어있는) 패드가
존재한다.
  2. 개구리가 움직인 후에 반드시 그 패드로 이동한다.

입력
1. 개구리의 수 n (1<= n <= 2* 10^5)
2. i번 개구리가 처음에 앉아 있는 패드의 위치를 나타내는 n개의 정수 x_i (1 <= x_i
<= 10^6)
  ...
  3. 개구리가 움직일 때마다,
    ...
    3. 그 패드로 이동한다.
      1. 패드 상태 배열에서 그 패드의 상태를 업데이트한다.
        1. 그 패드의 왼쪽에 있는 패드들 중, 기존에 그 패드를 가리키고 있었던
패드들을 모두 그 패드가 가리키고 있던 패드를 가리키게 한다.
      2. 기존 패드의 상태를 업데이트한다.
        1. 기존 패드의 왼쪽에 있던 패드 중 기존 패드보다 우측을 가리키고 있던
패드들을 모두 그 패드를 가리키게 한다.
      2. 개구리의 현 위치를 업데이트한다.
      3. 그 패드를 출력한다.
  worst: n*q?
...
int main(void) {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    auto next_pad =
        std::upper_bound(empty_pads.begin(), empty_pads.end(), x[jumped_frog]);
