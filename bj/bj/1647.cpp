...
disjoint_set::disjoint_set(int v) {
  root = new int[v + 1];
  std::memset(root, 0xFF,
              (v + 1) * sizeof(int));  // windows compatible sizeof array
}
disjoint_set::~disjoint_set(void) { delete[] root; }
...
bool disjoint_set::union_merge(int v1, int v2) {
  ...
  if (root[v1] > root[v2])
    std::swap(v1, v2);  // then v1 is the root of bigger tree
  root[v1] += root[v2];
  root[v2] = v1;
  return true;
}
...
bool edge_compare(const edge& a, const edge& b) { return a.w < b.w; }
