...
public:
	finder(std::vector<std::vector<char>>& map, std::set<char> keys)
		: map{ map }, search_stack{}, locked_door{}, document_num{ 0 }, keys{ keys },
		w{ static_cast<int>(map.size()) }, h{ static_cast<int>(map[0].size()) },
	...
	bool is_in_map(vec2 v) {
		return 0 <= v.x && v.x < w && 0 <= v.y && v.y < h;
	} 
	bool is_visited(vec2 v) {
		return visited[v.x][v.y];
	}
	void set_visited(vec2 v) {
		visited[v.x][v.y] = true;
	}
	char get_map_component(vec2 v) {
		return map[v.x][v.y];
	}
private:
	...
	void _search() {
		...
		vec2 v = search_stack.back(); search_stack.pop_back();
	...
	char is_door(char c) {
		if ('A' <= c && c <= 'Z') return std::tolower(c);
		else return 0;
	}
