...
class DisjointSet {
 ...
 private:
  int* root;  // root[i] means the root vertex of set in which i-th vertex is
              // included if root[i]<0, i-th vertex is root and -root[i] is the
              // size of the set
};
...
DisjointSet::~DisjointSet(void) { delete[] root; }
