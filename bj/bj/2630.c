...
int solve(int (*paper)[128], int x, int y, int step, int* white, int* blue) {
  // base
  if (step == 1) {
    if (paper[x][y] == BLUE)
      *blue += 1;
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      r[i][j] = solve(paper, x + step * i, y + step * j, step, white, blue);
    }
  }
  if (r[0][0] == BLUE && r[0][1] == BLUE && r[1][0] == BLUE &&
      r[1][1] == BLUE) {
    *blue -= 3;
    return BLUE;
  } else if (r[0][0] == WHITE && r[0][1] == WHITE && r[1][0] == WHITE &&
             r[1][1] == WHITE) {
    *white -= 3;
    return WHITE;
  } else {
