...
n개의 숫자(a1, a2, ..., an)가 모두 다르다는 것을 표현하는 한 줄의 표기법을 생각해 보라.
C(n, 2)개의 "!="가 필요할 것이다.
이 중 사전순으로 가장 앞에 오는 것을 출력하라.
...
3. 그 수를 제외하고, 아직 그 수와 명백한 부등 관계가 표현되지 않은 수를 선택한다.
	1. 단, 아직 모든 부등 관계가 표현되지 않았는데 다음 수를 찾지 못하면 이전으로 되돌아감.
4. 반복.
