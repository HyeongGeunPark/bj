...
int main(void) {
  ...
  while (!queue_is_empty(&q)) {
    ...
    while (len--) {
      ...
      if (z - 1 >= 0 && tom[x][y][z - 1] == 0) {
        queue_add_last(&q, x);
        queue_add_last(&q, y);
        queue_add_last(&q, z - 1);
        tom[x][y][z - 1] = res + 1;
        cnt--;
      }
      if (z + 1 < m && tom[x][y][z + 1] == 0) {
        queue_add_last(&q, x);
        queue_add_last(&q, y);
        queue_add_last(&q, z + 1);
        tom[x][y][z + 1] = res + 1;
