...
#define BUF_SIZE \
  100000  // size of input, output buffer for buffered IO functions
#define STDOUT_FD 1
#define STDIN_FD 0
...
#define bmap_init(name, bit)        \
  size_t name[bit_to_long(bit)] = { \
      0,                            \
  }
#define bmap_init_dynamic(name, bit) \
  size_t* name = calloc(bit_to_long(bit), sizeof(size_t))
#define bmap_free(name) free(name)
#define bmap_set(name, bit) \
  name[bit_to_index(bit)] |= (((size_t)1) << (bit_to_offset(bit)))
#define bmap_reset(name, bit) \
  name[bit_to_index(bit)] &= ~(((size_t)1) << (bit_to_offset(bit)))
#define bmap_toggle(name, bit) \
  name[bit_to_index(bit)] ^= (((size_t)1) << (bit_to_offset(bit)))
#define bmap_get(name, bit)                                          \
  ((int)((name[bit_to_index(bit)] >> ((size_t)bit_to_offset(bit))) & \
         ((size_t)1)))
#define bmap_set_long(name, bit) name[bit_to_index(bit)] |= BIT_FULL
#define bmap_reset_long(name, bit) name[bit_to_index(bit)] &= 0
#define bmap_reset_all(name, bit) \
  memset(name, 0, sizeof(size_t) * bit_to_long(bit))
#define bmap_set_all(name, bit) \
  memset(name, (char)0xFF, sizeof(size_t) * bit_to_long(bit))
...
#define __bmap2_size_init(name, a, b) \
  struct bmap2_size name_size = {.m = (b), .len = (a) * (b)}
// #define bmap2_init(name, n, m) __bmap2_size_init(name,n,m);bmap_init(name,
// name_size.len)
#define bmap2_init_dynamic(name, n, m) \
  __bmap2_size_init(name, n, m);       \
  bmap_init_dynamic(name, name_size.len)
#define bmap2_resize(name, a, b) \
  name_size.m = (b);             \
  name_size.len = ((a) * (b));   \
  free(name);                    \
  name = calloc(bit_to_long(name_size.len))
#define bmap2_free(name) \
  free(name);            \
  name_size = bmap2_size_zero
...
#define bmap2_toggle(name, i, j) \
  bmap_toggle(name, bmap2_index_to_bit(name, i, j))
...
#define bmap2_toggle_all(name) \
  for (int i = 0; i < (bit_to_long(name_size.len)); i++) name[i] ^= BIT_FULL
...
#include <string.h>
#define BUCKET_SIZE \
  150001  // this thing should be preferably bigger than x1.3 of total data
          // number
#define HASH_VAL 29
struct hashitem {
  char* key;
...
#define hashtable_init(name)                                  \
  struct hashtable name = {                                   \
      .items = calloc(BUCKET_SIZE, sizeof(struct hashitem*)), \
      .size = BUCKET_SIZE,                                    \
      .count = 0}
...
// delete hashtable
static inline void hashtable_free(struct hashtable* t) { free(t->items); }
...
#define hashtable_init_i(name)                                  \
  struct hashtable_i name = {                                   \
      .items = calloc(BUCKET_SIZE, sizeof(struct hashitem_i*)), \
      .size = BUCKET_SIZE,                                      \
      .count = 0}
...
// add an element
static inline void hashtable_add_i(struct hashtable_i* t, char* key,
                                   int value) {
...
// delete hashtable
static inline void hashtable_free_i(struct hashtable_i* t) { free(t->items); }
...
#define mm_heap_init(name) \
  struct mm_heap name = {.data = malloc(sizeof(int) * MM_HEAP_SIZE), .size = 0}
...
static inline int queue_is_empty(struct queue* q) { return (q->f == q->r); }
...
#define queue_len(name)                       \
  ((name.f - name.r) >= 0 ? (name.f - name.r) \
                          : (name.f - name.r + name.size + 1))
...
#define int2_init(name, nn, mm) \
  struct int2 name = {          \
      .n = (nn), .m = (mm), .data = calloc((nn) * (mm), sizeof(int))}
    ...
    for (j = 0; j < n; j++) {
      ...
      while (!queue_is_empty(&q)) {
        ...
        if (x > 0 && rgb[x - 1][y] == v) {
          ...
          queue_add_last(&q, y);
          queue_add_last(&q, rgb[x - 1][y]);
          rgb[x - 1][y] = 0;
        }
        if (y > 0 && rgb[x][y - 1] == v) {
          queue_add_last(&q, x);
          queue_add_last(&q, y - 1);
          queue_add_last(&q, rgb[x][y - 1]);
          rgb[x][y - 1] = 0;
        }
        if (x + 1 < n && rgb[x + 1][y] == v) {
          queue_add_last(&q, x + 1);
          queue_add_last(&q, y);
          queue_add_last(&q, rgb[x + 1][y]);
          rgb[x + 1][y] = 0;
        }
        if (y + 1 < n && rgb[x][y + 1] == v) {
          queue_add_last(&q, x);
          queue_add_last(&q, y + 1);
          queue_add_last(&q, rgb[x][y + 1]);
          rgb[x][y + 1] = 0;
