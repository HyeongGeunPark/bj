...
3. 맵에서 모든 키를 꺼내서, 도시와 주를 바꾸어 만든 키로 검색하면
  ...
  3-1. 도시 첫 2글자와 주 코드가 동일한 경우를 고려해야 한다. (e.g. CALABASAS
CA, CAMPBELL CA) 이런 도시들은 서로 special relation이 아니다. (동일 주에
있으므로) 3-2. 2로 나누기를 잊으면 안된다.
...
struct std::hash<city> {
  std::size_t operator()(const city& c) const {
    ...
    static_assert(sizeof(int) == sizeof(city),
...
int main(void) {
  ...
  for (auto it = map.begin(); it != map.end(); ++it) {
    if (it->first.state == it->first.name) continue;
