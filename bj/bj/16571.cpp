...
enum class game_turn {
	UNKNOWN = 0, PLAYER1 = 1, PLAYER2 = 2
}; 

inline uint8_t game_turn_2_uint8(game_turn turn) {
	if (turn == game_turn::PLAYER1) return 1;
	else if (turn == game_turn::PLAYER2) return 2;
} 
 ...
 * @return 게임이 어떻게 끝났는지?
 */
game_result check_if_game_ends(game_state state, coord pos, int empty_spaces_count) { 
...
game_result solve_tictactoe(game_state state, game_turn turn) {
	...
	for (int i = 0; i < empty_spaces_count; ++i) {
		...
		if (preferable_result == game_result::WIN)
			break; 
	} 
