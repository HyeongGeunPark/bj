...
int find_common_ancestor(int a, int b) {
  ...
  // make same depth
  for (int i = h; i >= 0; --i) {
    if ((1 << i) <= (depth_from_root[a] - depth_from_root[b])) a = parent[i][a];
  }
...
void deinit() { munmap(readBuf, st.st_size); }
...
void write_char(char c) { *writeCursor++ = c; }
...
int main() {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    */
    FastIO::write_int(distance_from_root[a] + distance_from_root[b] -
                      2 * distance_from_root[common_ancestor]);
    FastIO::write_char('\n');
  }
  FastIO::flush();
