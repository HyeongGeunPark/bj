...
enum {
	SUCCESS = 1, FAIL = 2, NOT_DETERMINED = 3
};
...
void solve(vec2 rr, vec2 bb) {
	...
	while (q.size()) {
		auto [r, b, depth, dir] = q.front(); q.pop();
		...
		case NOT_DETERMINED:
			...
			if (depth >= 10) break;
		// rotate and push
			for (int i = (dir + 1 == 4 ? 0 : dir + 1); i != dir; i = (i + 1 == 4 ? 0 : i + 1)) {
				r_next = rotate(r_next, i-1); b_next = rotate(b_next, i-1);
				if((i^dir)&1)
					q.push(std::tuple{ r_next, b_next, depth + 1, i });
			} 
		} 
	} 
}
