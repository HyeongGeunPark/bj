...
// dp 문제를 푸는 예시
// 부분집합의 합 문제: 집합의 부분집합 중 합이 특정 값인 부분집합이 존재하는지
// 찾는 문제.
...
bool subsetsum_backtrack(std::vector<int>& set, int sum, int i) {
  // 기저 조건들
  if (sum == 0)
    return true;  // 기저조건 1. target과 같은 합을 가지는 부분집합을 찾음
...
bool subsetsum_memoization(std::vector<int>& set, int sum, int i,
...
std::vector<std::vector<bool>> subsetsum_tabulation(std::vector<int>& set) {
  ...
  std::vector<std::vector<bool>> DP(set.size() + 1,
                                    std::vector<bool>(maxSum + 1, false));
...
void test_time(std::function<bool()> f, std::string s) {
  ...
  std::cout << "실행 시간: " << std::fixed << std::setprecision(9) << diff
            << "\n";
}
...
int main(void) {
  ...
  test_time([&]() -> bool { return subsetsum_backtrack(set, target, 0); },
            "backtrack, find " + std::to_string(target));
  test_time([&]() -> bool { return subsetsum_bruteforce(set, target); },
            "bruteforce, find " + std::to_string(target));

  // 상태 정의: i번째 원소까지 고려했을 때, 원소합이 j인 부분집합이 존재하는지
  // == memo[i][j]
  std::vector<std::vector<int>> memo(set.size(),
                                     std::vector<int>(7000, UNKNOWN));
  test_time(
      [&]() -> bool { return subsetsum_memoization(set, target, 0, memo); },
      "memoization, find " + std::to_string(target));
  test_time(
      [&]() -> bool {
        auto result = subsetsum_tabulation(set);
        return result[set.size()][target];
      },
      "tabulation, find " + std::to_string(target));
}