...
그 자리가 차있으면 그 자리에 있는 것을 해시테이블 2에 해시함수 2를 이용해
넣는다. 만일 그 자리도 차 있으면 그 자리에 있던 것을 다시 해시테이블 1에 넣는다.
...
class hash_map {
 ...
 public:
  ...
  void insert(int key) { insert_impl(key, 0, 1); }
  ...
  void print() {
    ...
    std::cout << "Data1: ";
    for (auto i : data1) std::cout << i << '\t';
    std::cout << std::endl;

    std::cout << "Data2: ";
    for (auto i : data2) std::cout << i << '\t';
    std::cout << std::endl;
  }
};
