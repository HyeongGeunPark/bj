...
int knapsack_01(int items, int capacity, std::vector<int> values,
...
int knapsack_inf0(int items, int capacity, std::vector<int> values,
                  std::vector<int> weight) {
  // 작동하지만 효율적인 방법은 아니다.
  std::vector<std::vector<int>> dp(items + 1,
                                   std::vector<int>(capacity + 1, 0));
...
int knapsack_inf1(int items, int capacity, std::vector<int> values,
                  std::vector<int> weight) {
...
int main(void) {
  ...
  values = {91, 81, 86, 64, 24, 61, 13, 57, 60, 25, 94, 54, 39, 62, 5,
            34, 95, 12, 63, 33, 63, 3,  42, 75, 65, 1,  84, 38, 46, 62};
  weight = {40, 13, 4,  17, 16, 36, 5,  33, 35, 16, 25, 29, 6,  28, 12,
            37, 26, 27, 32, 27, 7,  26, 5,  28, 39, 15, 38, 37, 15, 40};
