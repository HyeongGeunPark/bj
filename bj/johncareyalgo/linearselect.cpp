...
nth가 가리키는 원소가 range [first, last)를 완전히 정렬 한 후 그 위치에 있을
원소가 된다. O(n). partial 정렬된다.
...
template <typename RandomIt>
RandomIt partition_using_given_pivot(RandomIt begin, RandomIt end,
                                     RandomIt pivot) {
  auto left_iter = begin;
  auto right_iter = end - 1;

  while (true) {
    while (left_iter != right_iter && *left_iter < *pivot) ++left_iter;
    while (left_iter != right_iter && *right_iter >= *pivot) --right_iter;
  ...
  if (*pivot > *right_iter) std::iter_swap(pivot, right_iter);
...
RandomIt linear_time_select(RandomIt begin, RandomIt end, size_t i) {
  ...
  if (size > 0 && i < size) {
    ...
    auto median_of_medians =
        (M.size() == 1) ? M.begin()
                        : linear_time_select(M.begin(), M.end(), M.size() / 2);

    auto partition_iter =
        partition_using_given_pivot(begin, end, median_of_medians);
...
int main(void) { std::cout << *linear_time_select(arr.begin(), arr.end(), 10); }